...
class DebtChatScreen extends StatefulWidget {
  const DebtChatScreen({super.key});
  @override
  State<DebtChatScreen> createState() => _DebtChatScreenState();
}

class _DebtChatScreenState extends State<DebtChatScreen> with WidgetsBindingObserver {
  ...
  // messages: list of maps { role: 'user'|'assistant', text: String, ts: ISO string }
  List<Map<String, dynamic>> _messages = [];
  final int _maxHistoryItems = 300;
  ...
  Future<void> _initPrefsAndLoadHistory() async {
    ...
    _prefsKey = 'debt_chat_history_user_$uid';
    await _loadHistory();
  }
  ...
  Future<void> _loadHistory() async {
    try {
      ...
      debugPrint('Failed to load debt chat history: $e');
    }
  }
  ...
  Future<void> _saveHistory() async {
    try {
      ...
      debugPrint('Failed to save debt chat history: $e');
    }
  }
  ...
  Future<void> _clearHistoryConfirm() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Remove local debt chat history for this account? This cannot be undone.'),
        actions: [
  ...
  Future<void> _clearHistory() async {
    try {
      ...
      debugPrint('Failed to clear debt chat history: $e');
    }
  }
    ...
    await _speech.listen(
      onResult: (result) {
        try {
          ...
          final isFinal = result.finalResult == true || result.finalResult == 'final';
          _inputCtl.text = recognized;
          _inputCtl.selection = TextSelection.fromPosition(TextPosition(offset: _inputCtl.text.length));
          // optional: react on final result
        } catch (e) {
          debugPrint('Speech onResult parse error: $e');
        }
      },
      partialResults: true,
    );
        ...
        Row(mainAxisSize: MainAxisSize.min, children: [
          if (ts != '')
            Padding(
              ...
              child: Text(_formatTs(ts), style: const TextStyle(fontSize: 11, color: Colors.black38)),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Debt Assistant'),
          actions: [
            Row(children: [
